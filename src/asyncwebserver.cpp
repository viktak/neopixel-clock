...
{
    if (var == "numTrailLength")
        return (String)settings::trailLength;

    if (var == "chkReverse")
        return settings::reverseClockDirection == true ? "checked" : "";
    if (var == "chkfiveminute")
        return settings::showFiveMinuteMarks == true ? "checked" : "";
    if (var == "chkshowseconds")
        return settings::showSeconds == true ? "checked" : "";

    if (var == "numfiveMinuteMarkBrightness")
        return (String)settings::fiveMinuteMarkBrightness;
    if (var == "numledMaxBrightness")
        return (String)settings::ledMaxBrightness;
    if (var == "numlastofTrailBrightness")
        return (String)settings::lastOfTrailBrightness;
...
void POSTClockSettings(AsyncWebServerRequest *request)
{
    ...
    if (request->hasParam("numTrailLength", true))
    {
        p = request->getParam("numTrailLength", true);
        settings::trailLength = atoi(p->value().c_str());
    }

    if (request->hasParam("chkReverse", true))
    {
        p = request->getParam("chkReverse", true);
        settings::reverseClockDirection = true;
    }
    else
    {
        settings::reverseClockDirection = false;
    }

    if (request->hasParam("chkfiveminute", true))
    {
        p = request->getParam("chkfiveminute", true);
        settings::showFiveMinuteMarks = true;
    }
    else
    {
        settings::showFiveMinuteMarks = false;
    }

    if (request->hasParam("chkshowseconds", true))
    {
        p = request->getParam("chkshowseconds", true);
    ...
    if (request->hasParam("numfiveMinuteMarkBrightness", true))
    {
        p = request->getParam("numfiveMinuteMarkBrightness", true);
        settings::fiveMinuteMarkBrightness = atoi(p->value().c_str());
    }

    if (request->hasParam("numledMaxBrightness", true))
    {
        p = request->getParam("numledMaxBrightness", true);
        settings::ledMaxBrightness = atoi(p->value().c_str());
    }

    if (request->hasParam("numlastofTrailBrightness", true))
    {
        p = request->getParam("numlastofTrailBrightness", true);
        settings::lastOfTrailBrightness = atoi(p->value().c_str());
    }
...
void onOTAProgress(size_t current, size_t final)
{
    ...
    {
        Serial.printf("OTA: %3.0f%%\tTransfered %7u of %7u bytes\tSpeed: %4.3fkB/s\r\n", ((float)current / (float) final) * 100, current, final, ((float)(current - lastCurrent)) / 1024 * 2);
        lastCurrent = current;
